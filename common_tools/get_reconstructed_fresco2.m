...
function [im_rec_frags1,im_rec_frags2] = get_reconstructed_fresco2( im_fresco, frags_infos, frags_sol, interpolation_type, background_color )
    ...
    for k=1:length(frags_sol)
        ...
        for c=1:nb_channels
            ...
            non_intersect_intensities         = interp2(im_tmp, frag_coords_non_intersect(:,2), frag_coords_non_intersect(:,1), interpolation_type);
            intersect_intensities             = interp2(im_tmp, frag_coords_intersect(:,2), frag_coords_intersect(:,1), interpolation_type);
            im_tmp2(fresco_non_intersect_idx) = non_intersect_intensities;
            im_tmp2(fresco_intersect_idx)     = 0.5*(im_tmp2(fresco_intersect_idx) + intersect_intensities);
