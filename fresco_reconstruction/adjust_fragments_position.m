...
function [Exk,frags_sol] = eval_E( im_fresco_gray, im_fresco_alpha, frags_infos, frags_sol, xk_old, interpolation_type, ...
                        ...
                        if beta_sf>0 && (min_dist-frags_overlap_tolerance)>0 && (min_dist-frags_overlap_tolerance)<=extrapolation_distance
                            ...
                            if ~isempty(keep)
                                ...
                                if ~isempty(keep)
                                    ...
                                    E_sf                = eval_E_sf(frag_i_intensities, frag_j_intensities, size(frag_i_intensities,1), nb_channels, lambda2, mu2);
    ...
    disp(sprintf('#E_sf>0     = %d', numel(find(all_E_sf>0))));
    disp(sprintf('#E_sf<0     = %d', numel(find(all_E_sf<0))));
    disp(sprintf('mean E_sf   = %f', mean(all_E_sf<0)));
    disp(sprintf('median E_sf = %f', median(all_E_sf<0)));
