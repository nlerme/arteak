...
function frags_sol = adjust_fragments_position( im_fresco_gray, im_fresco_alpha, im_fresco_grads, frags_infos, frags_sol, general_parameters, mpp_parameters, gen_parameters, geometric_constraints )
    % We set parameters
...
function [Exk,frags_sol] = eval_E( im_fresco_gray, im_fresco_alpha, frags_infos, frags_sol, xk_old, interpolation_type, ...
    ...
    for i=1:numel(frags_sol)
        ...
        for j=1:numel(frags_sol)
            ...
            % enabling the term E_sf (worst case among all pairse energy 
            % % terms, we can safely skip this couple of fragments
            ...
            % We add the contribution of the terms E_no and/or E_sf
            % -----------------------------------------------------
                ...
                if ~isempty(keep)
                    ...
                    if ~isempty(frag_i_coords_e_Pwj)
                        ...
                        if beta_sf>0 && (min_dist-frags_overlap_tolerance)>0 && (min_dist-frags_overlap_tolerance)<=extrapolation_distance
                            % We project the pixel coordinates of the ith dilated fragment domain (d(P_wi)) into the frame of the jth dilated fragment domain (d(P_wj))
                            ...
                            % If the fragment P_wi does not lie outside P_wj, we add the contribution of E_sf
                            if ~isempty(keep)
                                % We compute the term E_sf
                                frag_i_coords_d_Pwi = frag_i_coords_d(keep,:);
                                frag_i_coords_d_Pwj = frag_i_coords_d_Pwj(keep,:);
                                frag_i_intensities  = get_intensities(im_frag_i_gray_ext, frag_i_coords_d_Pwi, interpolation_type);
                                frag_j_intensities  = get_intensities(im_frag_j_gray_ext, frag_i_coords_d_Pwj, interpolation_type);
                                E_sf                = eval_E_sf(frag_i_intensities, frag_j_intensities);
...
function E_sf = eval_E_sf( frag_i_intensities, frag_j_intensities )
