...
function run_reconstructions()
    ...
    function main()
        ...
        data_root_dir    = ['..' filesep 'data' filesep 'DB1'];
        verbose          = 1;
        show_figures     = 'off';
        ...
        run_reconstruction(data_root_dir, fresco_name, config_name, results_root_dir, verbose, show_figures);
...
%         end
        ...
        %---------------------- DB2 dataset ----------------------
        %results_root_dir = ['..' filesep 'test_results'];
        %data_root_dir    = ['..' filesep 'data' filesep 'DB2'];
        %verbose          = 0;
        %show_figures     = 'off';

        %--------------

        %fresco_name = 'Domenichino_Virgin-and-unicorn';
        %config_name = '';

        %fresco_name = 'Andrea-di-Bonaiuto';
        %config_name = '';

        %fresco_name = 'Zuccari_Cardinal-hat';
        %config_name = '';
...
%             fresco_name = a{end};
%             disp(sprintf('+ %s (%d/%d)', fresco_name, i, numel(dirs1)));
%             run_reconstruction(data_root_dir, fresco_name, '', results_root_dir, verbose, show_figures);
%         end
    end
    ...
    function run_reconstruction( data_root_dir, fresco_name, config_name, results_root_dir, verbose, show_figures )
        ...
        recompute_mpp_optimization            = 1;          % Flag indicating if MPP-based optimization is recomputed or loaded
        ...
        fresco_fns = get_matching_files(fresco_dir, '.*\..*');

        if isempty(fresco_fns)
            disp('error: unable to load fresco image. Wrong path, empty directory or unknown file format?');
            return;
        end
        ...
        if ~isempty(config_name)
            ...
            frags_dir   = [fresco_dir filesep 'frag_eroded'];
            gt_fn       = [fresco_dir filesep 'fragments.txt'];
        ...
        if recompute_preprocessing
            % We load fresco
            msg('+ loading of fresco', verbose);
            im_fresco_color  = imread(fresco_fn);
            ...
            % We load fragments and compute some elements about them
            msg('+ preprocessing of fragments', verbose);
            frag_fns = get_matching_files(frags_dir, '.*\.png');
            %frag_fns{numel(frag_fns)+1} = 'my_eroded_fragment.png'; % DEBUGAGE, A RETIRER ABSOLUMENT A TERME !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

            if isempty(frag_fns)
                disp('error: unable to load fragments. Wrong path, empty directory or unknown file format?');
                return;
            end
            ...
            parfor k=1:numel(frag_fns)
                ...
                %figure, imshow(im_frag_color_ext,[]);
                %imwrite(im_frag_color, sprintf('input_%03d.jpg', k));
                %imwrite(im_frag_color_ext, sprintf('output_%03d.jpg', k));
                %-----------------------------------
                ...
                %------ For debugging ------
                %im_tmp = zeros(frag_size);
                %im_tmp(round(inner_circle_center(1)),round(inner_circle_center(2))) = 1;
                %im_tmp2 = max(0,(bwdist(im_tmp>0, 'euclidean')<inner_circle_radius)-logical(im_frag_alpha)); 
                %figure, imshow(im_tmp2,[]);
                %if sum(im_tmp2(:))>0
                %   disp('inaccurate inner circle .............');
                %end

                %im_tmp = zeros(frag_size);
                %im_tmp(round(outer_circle_center(1)),round(outer_circle_center(2))) = 1;
                %im_tmp2 = max(0,logical(im_frag_alpha)-(bwdist(im_tmp>0, 'euclidean')<outer_circle_radius));
                %figure, imshow(im_tmp2,[]);
                %if sum(im_tmp2(:))>0
                %   disp('inaccurate outer circle .............');
                %end

                %figure('units','normalized','outerposition',[0 0 1 1],'visible','off');
                %imshow(im_frag_color,[]);
                %hold on;
                %viscircles(flip(inner_circle_center), inner_circle_radius, 'Color', 'green');
                %viscircles(flip(outer_circle_center), outer_circle_radius, 'Color', 'red');
                %fn = sprintf('toto/fragment_%d.png', k);
                %saveas(gcf, fn);
            ...
            if ~consistency
                error('error: the number of channels of any fragment image must be the same as the fresco image');
                return;
            end
            ...
            if ~exist(gt_fn)
                ...
                if numel(id)~=numel(tx) || numel(id)~=numel(ty) || numel(id)~=numel(angles)
                    disp('error: the format of the ground truth file is incorrect');
                    return;
                end
            ...
            if ~exist(preprocessing_fn)
                msg(sprintf('error: unable to load ''%s''', preprocessing_fn), verbose);
                return;
            end
...
%         return;
        ...
        if recompute_color_matching
            ...
            if ~exist(color_matching_fn)
                msg(sprintf('error: unable to load ''%s''', color_matching_fn), verbose);
                return;
            end
        ...
        if recompute_features_matching
            ...
            if ~exist(features_matching_fn)
                fprintf('error: unable to load ''%s''', features_matching_fn);
                return;
            end
...
%             end
            ...
            if ~exist(mpp_optimization_fn)
                fprintf('error: unable to load ''%s''', mpp_optimization_fn);
                return;
            end
