...
%         end
    ...
    function run_reconstruction( data_root_dir, fresco_name, config_name, results_root_dir, verbose, show_figures )
        ...
        if isempty(fresco_fns)
            error('Unable to load fresco image. Wrong path, empty directory or unknown file format?');
        end
        ...
        if recompute_preprocessing
            ...
            if isempty(frag_fns)
                error('Unable to load fragments. Wrong path, empty directory or unknown file format?');
            end
