...
typedef GCoptimization::DataCostFunctor DataCostFunctor;
typedef GCoptimization::SmoothCostFunctor SmoothCostFunctor;

struct MyDataCostFunctor : public DataCostFunctor
{
    const char *m_FuncName;

    MyDataCostFunctor( const char *func_name ) : m_FuncName(strdup(func_name)){}

    ~MyDataCostFunctor(){ delete m_FuncName; }

    virtual EnergyTermType compute( SiteID s, LabelID l )
    {
        mwSize outdim = 1;
        mxArray* mx_s = mxCreateNumericArray(1, &outdim, cSiteClassID, mxREAL); *(SiteID*)mxGetData(mx_s) = (s+1);
        mxArray* mx_l = mxCreateNumericArray(1, &outdim, cLabelClassID, mxREAL); *(SiteID*)mxGetData(mx_l) = (l+1);
        mxArray* plhs[1] = {0};
        mxArray* prhs[2] = {mx_s,mx_l};
        mexCallMATLAB(1, plhs, 2, prhs, m_FuncName);
        return *(EnergyTermType*)mxGetData(plhs[0]);
    }
};

struct MySmoothCostFunctor : public SmoothCostFunctor
{
    const char *m_FuncName;

    MySmoothCostFunctor( const char *func_name ) : m_FuncName(strdup(func_name)){}

    ~MySmoothCostFunctor(){ delete m_FuncName; }

    virtual EnergyTermType compute( SiteID s1, SiteID s2, LabelID l1, LabelID l2 )
    {
        mwSize outdim = 1;
        mxArray* mx_s1 = mxCreateNumericArray(1, &outdim, cSiteClassID, mxREAL); *(SiteID*)mxGetData(mx_s1) = (s1+1);
        mxArray* mx_s2 = mxCreateNumericArray(1, &outdim, cSiteClassID, mxREAL); *(SiteID*)mxGetData(mx_s2) = (s2+1);
        mxArray* mx_l1 = mxCreateNumericArray(1, &outdim, cLabelClassID, mxREAL); *(LabelID*)mxGetData(mx_l1) = (l1+1);
        mxArray* mx_l2 = mxCreateNumericArray(1, &outdim, cLabelClassID, mxREAL); *(LabelID*)mxGetData(mx_l2) = (l2+1);
        mxArray* plhs[1] = {0};
        mxArray* prhs[4] = {mx_s1,mx_s2,mx_l1,mx_l2};
        mexCallMATLAB(1, plhs, 4, prhs, m_FuncName);
        return *(EnergyTermType*)mxGetData(plhs[0]);
    }
};

GCO_EXPORT(gco_setdatacost2)
{
    MATLAB_ASSERT_ARGCOUNT(0,2);
    MATLAB_ASSERT_HANDLE(1);
    GCInstanceInfo& gcinstance = sGetGCInstance(*(int*)mxGetData(prhs[0]));
    mwSize func_name_length = mxGetN(prhs[1])*sizeof(mxChar)+1;
	char func_name[512]={0};
	mxGetString(prhs[1], func_name, func_name_length);
    DataCostFunctor *dcf = new MyDataCostFunctor(func_name);
    //printf("data cost = %f\n", dcf->compute(0, 4));
    gcinstance.gco->setDataCostFunctor(dcf);
}

GCO_EXPORT(gco_setsmoothcost2)
{
    MATLAB_ASSERT_ARGCOUNT(0,2);
    MATLAB_ASSERT_HANDLE(1);
    GCInstanceInfo& gcinstance = sGetGCInstance(*(int*)mxGetData(prhs[0]));
	...
	char func_name[512]={0};
	mxGetString(prhs[1], func_name, func_name_length);
    SmoothCostFunctor *scf = new MySmoothCostFunctor(func_name);
    //printf("cost = %f\n", scf->compute(0, 0, 1, 4));
    gcinstance.gco->setSmoothCostFunctor(scf);
}
