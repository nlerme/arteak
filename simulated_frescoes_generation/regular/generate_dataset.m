...
function generate_dataset()
    ...
    function main()
        ...
        fragments_erosion_rates         = [0.0,0.003,0.006];                                                                              % Amount of erosion in percentage of the smallest size of the fresco image (vector with entries in [0,1[)
        fragments_missing_rates         = [0.0,0.2,0.4];                                                                                  % Percentages of missing fragments (vector with entries in ]0,1])
        fragments_spurious_rates        = [0.0,0.2,0.4];                                                                                  % Percentages of spurious fragments (vector with entries in [0,1[)
        ...
        for i=1:numel(input_frescoes_fns)
            ...
            for j=1:numel(fragments_sizes)
                % Message
                fragment_size = 2*floor(min(fresco_size)*fragments_sizes{j}*0.5)+1; % make fragment size as odd integers to cope with the problem of their accurate placement
                msg(sprintf('  + fragment size=[%d,%d]', fragment_size(1), fragment_size(2)), verbose);

                % We loop over fragments (zero padding of the fresco)
                ...
                frags_coords       = cell(1,nb_total_frags);
                frags_translations = cell(1,nb_total_frags);
                frags_angles       = cell(1,nb_total_frags);
                available_angles   = fragments_rotation_angles;
                ...
                available_translations = frags_translations;
                ...
                for k=1:numel(fragments_missing_rates)
                    ...
                    for l=1:numel(fragments_spurious_rates)
                        ...
                        if spurious_rate>0
                            ...
                            frags_coords3       = frags_coords3(rp);
                            spurious_idx        = [];
                            true_idx            = [];
                        ...
                        for m=1:numel(fragments_noise_stds)
                            ...
                            for n=1:numel(fragments_palette_sizes)
                                ...
                                for o=1:numel(fragments_erosion_rates)
                                    ...
                                    config_dir        = [fresco_dir filesep sprintf('%s_%d_%d_%d_%.2f_%d_%d', fresco_name, fragment_size(1), round(missing_rate*100), round(spurious_rate*100), noise_std, palette_size, erosion_radius)];
                                    ...
                                    % We simulate degradations on the fragment images
                                    ...
                                    save_gen_parameters({'palette_size', 'fragment_size', 'missing_rate', 'spurious_rate', 'mean_frags_gap', 'noise_std', ...
                                                         'min_scale_factor', 'max_scale_factor', 'min_erosion_factor', 'max_erosion_factor', ...
                                                         'padding_factor', 'interpolation_type', 'nb_attempts'}, ...
                                                        {uint64(palette_size), uint64(max(fragment_size)), double(missing_rate*100), double(spurious_rate*100), ...
                                                         double(2*mean(erosion_radii)), double(noise_std), ...
                                    ...
                                    for n=1:numel(available_translations)
                                        translation = available_translations(n);
                                        translation = translation{1};
                                        geometric_constraints.locations = [geometric_constraints.locations;translation];
                                    end

                                    geometric_constraints.orientations = available_angles;
                                    save_geometric_constraints(geometric_constraints, constraints_fn);

                                    % We construct the solution composed of fragments
                                    frags_sol = create_fragments_solution(frags_coords3, frags_translations3, frags_angles3, true_idx);
