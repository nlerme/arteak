...
function generate_dataset()
    ...
    function main()
        ...
        fragments_erosion_rates         = [0.0,0.006];                                                                                        % Percentages of fragments erosion, relatively to the fresco image size (vector with entries in [0,1])
        ...
        fresco_fragmentation_min_dists  = [0.05,0.15];                                                                                        % Minimum distances between sampled location of fragments, relatively to the fresco image size (vector of positive reals)
        ...
        fresco_fragmentation_params     = struct('sampling_min_dist', [], 'sampling_nb_points', [], 'sampling_type', 'non-uniform-lloyd', ...
                                                 'noise_exponent', 1.7, 'uncertainty_band_size', [], 'uncertainty_band_ratio', 0.02, ...
                                                 'beta', 0.5, 'dist_name', 'euclidean', 'dist_weights', [0.5,1.0]);                           % Parameters for simulating fragmentation of frescoes (struct)
        ...
        % We check if parameters are consistent
        if numel(fresco_fragmentation_min_dists)~=numel(fresco_fragmentation_nb_frags)
            error('The number of minimum distances and number of fragments must be the same');
        end
        ...
        for i=1:numel(input_frescoes_fns)
            ...
            for j=1:numel(fresco_fragmentation_min_dists)
                ...
                fresco_fragmentation_min_dist2 = fresco_fragmentation_min_dist*min(fresco_size);
                fresco_fragmentation_nb_frags2 = fresco_fragmentation_nb_frags(j);

                % We add them to the parameters structure
                fresco_fragmentation_params.uncertainty_band_size = fresco_fragmentation_params.uncertainty_band_ratio*min(fresco_size);
                fresco_fragmentation_params.sampling_min_dist     = fresco_fragmentation_min_dist2;
                fresco_fragmentation_params.sampling_nb_points    = fresco_fragmentation_nb_frags2;

                % Message
                msg(sprintf('  + minimum distance=%f (nb frags=%d)', fresco_fragmentation_min_dist, fresco_fragmentation_nb_frags2), verbose);
